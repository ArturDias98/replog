...
import { ExportImportPort, ExportResult, ImportResult, StoragePort } from '@replog/application';

@Injectable()
export class ExportImportServiceImpl extends ExportImportPort {
    ...
    async importWorkouts(file: File): Promise<ImportResult> {
        try {
            ...
            const currentWorkouts = await this.storage.loadAll();
            ...
            await this.storage.saveAll([...currentWorkouts, ...newWorkouts]);
